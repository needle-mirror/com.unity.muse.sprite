    ...
    internal class Painter : IDisposable
    {
        ...
        public void InitializeWithData(byte[] imageData)
        {
            ...
            m_RenderTexture = new CustomRenderTexture(m_Size.x, m_Size.y)
            {
                ...
                material = m_PaintMaterial.material,
            ...
            if (isClear)
                m_RenderTexture.initializationColor = Color.clear;
            else
                m_RenderTexture.initializationTexture = m_Texture;
        ...
        public void Paint(Vector2 startPosition, Vector2 endPosition, Color color)
        {
            ...
            SetUpdateZones(startPosition, endPosition);

            m_PaintMaterial.SetRadius(brushRadius);
        ...
        void SetUpdateZones(Vector2 startPosition, Vector2 endPosition)
        {
            var enlargedBrushRadius = brushRadius + 1.0f;
            var minX = Mathf.Max(Mathf.RoundToInt(Mathf.Min(startPosition.x, endPosition.x) - enlargedBrushRadius), 0);
            var maxX = Mathf.Min(Mathf.RoundToInt(Mathf.Max(startPosition.x, endPosition.x) + enlargedBrushRadius), m_Size.x - 1);
            var minY = Mathf.Max(Mathf.RoundToInt(Mathf.Min(startPosition.y, endPosition.y) - enlargedBrushRadius), 0);
            var maxY = Mathf.Min(Mathf.RoundToInt(Mathf.Max(startPosition.y, endPosition.y) + enlargedBrushRadius), m_Size.y - 1);

            var size = new Vector3(maxX - minX + 1, maxY - minY + 1);
            var center = new Vector3((minX + maxX + 1) * 0.5f, m_Size.y - (minY + maxY + 1) * 0.5f);

            m_RenderTexture.SetUpdateZones(new[] { new CustomRenderTextureUpdateZone { updateZoneCenter = center, updateZoneSize = size } });
            m_RenderTexture.Update();
        }
        ...
        {
            m_RenderTexture.initializationColor = Color.white;
            m_RenderTexture.initializationTexture = m_Texture;
            m_RenderTexture.Initialize();
        }
